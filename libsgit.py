...
## This is a simple implementation of a git-like CLI


## The command line interface is implemented using argparse.
argparser = argparse.ArgumentParser(description="Might be The stupidest content tracker")

argsubparsers = argparser.add_subparsers(title="Commands", dest="command")
# next line is a flag requiring a subcommand always
argsubparsers.required = True


# this is the main function, which is called when the programme is run
    # it parses the command line arguments and calls the appropriate function
    # there are 15 commands, each with its own function
...
def repo_default_config():
    rdf = configparser.ConfigParser()

    rdf.add_section("core")
    rdf.set("core", "repositoryformatversion", "0")
    rdf.set("core", "filemode", "false")
    rdf.set("core", "bare", "false")
...
#Find an object
    # This function is defined later, as it is heviley used in the other
    # functions, and we don't want to have to define it all for now
...
def object_find(repo, name, fmt=None, follow=True):
    ...
    if len(sha) > 1:
        # Ambiguous name.  We need to disambiguate.
        if not fmt:
            raise Exception(f"Ambiguous name {name} (multiple candidates)")
        # We have a format, so we can disambiguate.
        for s in sha:
            obj = object_read(repo, s)
            if obj.fmt == fmt:
                return s
        raise Exception(f"Ambiguous name {name} (no candidate of type {fmt})")
    sha = sha[0]
    if not fmt:
        return sha
    while True:
...
# This is a so-called a bridge function, all commands shall follow suit.
def cmd_init(args):
    repo_create(args.path)
    # Noted is the fact that repo initialization is an abstraction of its creation
...
def cat_file(repo, obj, fmt=None):
    """Display the content of an object in the repository."""
...
# This function parses the key-value list format used by git, which
# is used for commits and tags.  
...
# Serilaziation is a process used to convert the object into a stream
...
# The log command
# This command is used to display the history of a given commit
...
# This command is used to checkout a commit inside of a directory
...
# This is used to create a ref identifier
def ref_resolve(repo, ref):
    ...
    if data.startswith("ref: "):
        ...
        return data
# Recursively collect all references in a directory    
...
# The tag command
# This command is used to create a tag object, or a lightweight tag
argsp = argsubparsers.add_parser(
    "tag",
...
def tag_create(repo, name, ref, create_tag_object=False):
    ...
    if create_tag_object:
        ...
        tag.kvlm[b'tagger'] = b'wmwyag <wmwyag@example.com>'
        # …and a tag message!
        tag.kvlm[None] = b"A tag generated by wmwyag, which won't let you customize the message!\n"
...
argsp.add_argument("--wmwyag-type",
                   metavar="type",
                   dest="type",
                   choices=["blob", "commit", "tag", "tree"],
                   default=None,
...
# Read the index file (not so pretty)
def index_read(repo):
    ...
    assert version == 2, "wmwyag only supports index file version 2"
...
def add(repo, paths, delete=True, skip_missing=False):
    ...
    # optimal, good enough for wmwyag!)
    #
